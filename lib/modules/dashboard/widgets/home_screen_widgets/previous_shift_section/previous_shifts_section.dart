...
import 'package:orion_safeguard/modules/dashboard/widgets/home_screen_widgets/previous_shift_section/pervious_shifts_loading_view.dart';
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeCubit, HomeState>(
        ...
        builder: (context, state) {
          ...
          if (state.previousShifts.status == Status.error) {
            return SizedBox();
          }
          return Column(
            children: [
              CustomSeeAllRowWidget(
                ...
                fun: () {
                  context.read<DashboardCubit>().changePage(2);
                },
              ),
              h1,
              if (state.previousShifts.status == Status.loading)
                PreviousShiftsLoadingView()
              else
                Container(
